...
mysqladmin -u "$db_user" -p"$db_pass" ping
if [ $? -ne 0 ]; then
  echo "Erreur: Impossible de se connecter à MySQL avec l'utilisateur $db_user"
  exit 1
fi
...
if [ $? -ne 0 ]; then
  echo "Erreur: Impossible de créer la base de données $db_name"
  exit 1
fi

# Accorder les privilèges
mysql -u "$db_user" -p"$db_pass" -e "GRANT ALL PRIVILEGES ON $db_name.* TO '$db_user'@'localhost' IDENTIFIED BY '$db_pass'"
if [ $? -ne 0 ]; then
  echo "Erreur: Impossible d'accorder les privilèges sur $db_name à $db_user"
  exit 1
fi
...
if [ $? -ne 0 ]; then
  echo "Erreur: Impossible de rafraîchir les privilèges"
  exit 1
fi
